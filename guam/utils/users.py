...
from .autofs import add_autofs_filesystem
...
"""
        ...
        if secondary_gid:
            for j in secondary_gid:
                samdb.add_remove_group_members(
                    groupname=j, members=[user.username], add_members_operation=True
                )
        ...
        add_autofs_filesystem(user.username, user.userafsserver, newuid, gid)
