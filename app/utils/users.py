...
def add_user(samdb: SamDB, user: User):
    ...
    try:
        ...
        samdb.newuser(username=user.username,password=password)
    except:
 ...
 """
    ...
    sp = subprocess.Popen(
        [adduser_command], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    command_result = sp.stdout.read().decode()

    addtoprimarygroup = (
        "sudo samba-tool group addmembers '"
        + user.userprimarygroup
        + "' "
        + user.username
    )
    primarygroupaddcommand = subprocess.Popen(
        [addtoprimarygroup],
        shell=True,
 ...
 distinguishedName: CN=/var/www/nextcloud-data/{user.username},CN=auto.Nextcloud,OU=Nextcloud,OU=AutoFS,DC=grit,DC=ucsb,DC=edu"""
    ...
    if "ERROR(ldb): Failed to add user" in command_result:
        ...
        ldif_result = "SUCCESS"
        adduserafsldif = (
            "sudo ldbadd -H /var/lib/samba/private/sam.ldb /tmp/userafsadd.ldif"
        )
 ...
 """
    sshProcess = subprocess.Popen(
        ["ssh", "-T", server, nfscommand], stdin=subprocess.PIPE, stdout=subprocess.PIPE
    )
    sshProcess_result = sshProcess.stdout.read().decode()
    # result = (
    #     "form input: "
    #     + str(result)
    #     + " user add command result: "
    #     + command_result
    #     + " home folder autofs command result: "
    #     + ldif_result
    #     + " group add command output: "
    #     + groupadd_result
    #     + " home folder creation result: "
    #     + sshProcess_result
    # )
    print("here")
    return user
